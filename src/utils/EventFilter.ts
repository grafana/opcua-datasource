import { FilterOperand, FilterOperandSer, EventFilter, EventFilterSer, LiteralOp, FilterOperator, FilterOperandEnum, ElementOp } from "../types";
...
export function serializeEventFilter(eventFilter: EventFilter): EventFilterSer {
    return {
        oper: eventFilter.oper, operands: eventFilter.operands.map(evf => serializeEventOperand(evf))
    };
}
...
export function deserializeEventFilter(eventFilter: EventFilterSer): EventFilter {
    return {
        oper: eventFilter.oper, operands: eventFilter.operands.map(evf => deserializeEventOperand(evf))
    };
}
