...
export class QueryEditor extends PureComponent<Props, State> {
  ...
  onExpand = (nodeId: string) => {
    console.log('onExpand', nodeId);
    const { dataSource } = this.state;
    const entry = dataSource.find(entry => entry.key === nodeId);
    if (entry) {
      this.props.datasource
        .browse(nodeId)
        .then((results: OpcUaBrowseResults[]) => {
          entry.children = results.map((item: OpcUaBrowseResults) => {
            return {
              key: item.nodeId,
        ...
        .then((ds: TransferItem[]) => this.updateDataSource(ds));
    }
  };
